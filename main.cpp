...
public:
  Employee(string n, int i) : name(n), id(i) { totalActiveEmployees++; }

  // Virtual destructor for safe dynamic memory cleanup
  virtual ~Employee() { totalActiveEmployees--; }
  ...
  static void setBudget(double b) { companyBudget = b; }
...
public:
  ...
  virtual double calculateMonthlyPay() { return annualSalary / 12.0; }
};
...
public:
  ...
  double simulateMonth() override {
    cout << "[WORK] Senior Engineer " << name
         << " designed system architectures this month.\n";
    return calculateMonthlyPay();
  }

  void displayDetails() override {
    cout << "ID: " << id << " | Name: " << name
         << " | Role: Senior Engineer | Base: ₹50 LPA\n";
  }
};
...
public:
  ...
  double simulateMonth() override {
    cout << "[WORK] Junior Engineer " << name
         << " completed code sprints and training this month.\n";
    return calculateMonthlyPay();
  }

  void displayDetails() override {
    cout << "ID: " << id << " | Name: " << name
         << " | Role: Junior Engineer | Base: ₹20 LPA\n";
  }
};
...
public:
  ...
  double simulateMonth() override {
    cout << "[WORK] Manager " << name
         << " handled department operations and planning this month.\n";
    return calculateMonthlyPay();
  }

  void displayDetails() override {
    cout << "ID: " << id << " | Name: " << name
         << " | Role: Department Manager | Base: ₹30 LPA\n";
  }
};
...
public:
  ...
  double simulateMonth() override {
    cout << "[WORK] Floor Tech " << name
         << " logged 160 hours on the assembly floor this month.\n";
    return calculateMonthlyPay();
  }

  void displayDetails() override {
    cout << "ID: " << id << " | Name: " << name
         << " | Role: Floor Tech | Rate: ₹" << hourlyRate << "/hr\n";
  }
};
...
int main() {
  ...
  while (running) {
    cout << "\n==========================================\n";
    cout << "ACTIVE EMPLOYEES: " << Employee::getTotalActive() << " / "
         << maxCapacity << "\n";
    cout << "COMPANY BUDGET: ₹" << fixed << setprecision(2)
         << Employee::getBudget() << "\n";
    cout << "==========================================\n";
    ...
    if (choice == 1) {
      ...
      if (roleChoice == 1)
        roster[currentHeadcount] = new SeniorEngineer(name, nextId);
      else if (roleChoice == 2)
        roster[currentHeadcount] = new JuniorEngineer(name, nextId);
      else if (roleChoice == 3)
        roster[currentHeadcount] = new Manager(name, nextId);
      else if (roleChoice == 4)
        roster[currentHeadcount] = new FloorTechnician(name, nextId);
      else {
        cout << ">> Invalid role.\n";
        continue;
      }
      ...
      cout << ">> SUCCESS: " << name
           << " hired successfully. ID assigned: " << nextId << "\n";
      currentHeadcount++;
      nextId++;
    } else if (choice == 2) {
      ...
      for (int i = 0; i < currentHeadcount; i++) {
        if (roster[i] != nullptr) {
          ...
          // polymorphism : This one call prints the specific work done AND
          // calculates the correct pay
          double pay = roster[i]->simulateMonth();

          cout << "   -> Monthly Salary Credited: ₹" << fixed << setprecision(2)
               << pay << "\n\n";
          totalPayout += pay;
        }
      }
      ...
      cout << ">> TOTAL MONTHLY PAYROLL DEDUCTED: ₹" << fixed << setprecision(2)
           << totalPayout << "\n";
